...
C  
     ...
     *            asmall  = 2.d-16,pi=3.1415926d0)	 
      real*8 FibreSheet(10000,7), PDFdata(1500,4), Fb(3,3), sigmaa(3,3)
...
c- I_8 and I_4S
			...
			do i=1,3
				do j=1,3
					...
					psts(i,j)=pFb(i,2)*pFb(j,2)
					pftstf(i,j)=pFb(i,1)*pFb(j,2)+pFb(i,2)*pFb(j,1)
				end do
			end do 
		
			p_I4s=pC(2,2)
			p_I4sa=max(p_I4s,1.0d0)
			p_I8fs=pC(1,2)
			

			psigmaa=psigmaa+(Pp_afs*p_I8fs*exp(Pp_bfs*p_I8fs**2))*(pftstf-2.0d0*p_I8fs/3.0d0*pIden)
     *         +2.0d0*Pp_as*(p_I4sa-1.0d0)*exp(Pp_bs*(p_I4sa-1.0d0)**2)*(psts(i,j)-p_I4sa/3.0d0*pIden(i,j))
...
!            close(401)		
			...
			open(402,FILE='D:/DBGuan/HaoSpecialIssue/HumanHeart/Temperature/FibreDirection/PDF_b1b24539_human.txt',STATUS='OLD')
            read(402,*) PDFelement
