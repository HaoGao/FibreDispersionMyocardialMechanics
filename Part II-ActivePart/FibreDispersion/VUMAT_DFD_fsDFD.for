...
C  
     ...
     *            asmall  = 2.d-16,pi=3.1415926d0)	 
      real*8 FibreSheet(10000,7), PDFdata(1500,5), PDFdata2(1500,5),Fb(3,3), sigmaa(3,3)
      integer*4 PDFelement, StartAnalysis
      common /FibreSheetblk/FibreSheet, PDFdata,PDFdata2, PDFelement, StartAnalysis
...
C     Possible values for the lOp argument
      ...
      real*8 FibreSheet(10000,7), PDFdata(1500,5) , PDFdata2(1500,5) 
      integer*4 PDFelement, StartAnalysis
      common /FibreSheetblk/FibreSheet, PDFdata, PDFdata2, PDFelement, StartAnalysis
...
C-dispersed fibre 
		do kk=1, PDFelement
			do i=1,3
				pNn(i)=PDFdata(kk,i+1)
				psr(i)=PDFdata2(kk,i+1)
c-				pnr(i)=PDFdata(kk,i+1)
			end do
...
c-					pntn(i,j)=pnNn(i)*pnNn(j)
			...
			if (p_I4sa .GE. 1.0d0) then
				psigmaa=psigmaa+PDFdata2(kk,1)*2.0d0*Pp_as*(p_I4sa-1)*exp(Pp_bs*((p_I4sa-1)**2))*(psts-p_I4sa/3.0d0*pIden)
			end if
...
!            close(401)		
			...
			open(402,FILE='D:/DBGuan/HaoSpecialIssue/HumanHeart/Temperature/FibreDirection/PDF80_b1b24500_fibre.txt',STATUS='OLD')
            read(402,*) PDFelement
            write(*,*) 'total data to read: n = ', PDFelement

            do i = 1 , PDFelement
                read(402, *) PDFdata(i,1),PDFdata(i,2),PDFdata(i,3),PDFdata(i,4)
				write(*, *) PDFdata(i,1),PDFdata(i,2),PDFdata(i,3),PDFdata(i,4)
            end do 
            write(*,*) 'read ,PDFdata(i,3) value finished!'
            close(402)	
	   

			open(401,FILE='D:/DBGuan/HaoSpecialIssue/HumanHeart/Temperature/FibreDirection/PDF80_b1b24500_sheet.txt',STATUS='OLD')
            read(401,*) PDFelement
            write(*,*) 'total data to read: n = ', PDFelement

            do i = 1 , PDFelement
                read(401, *) PDFdata2(i,1),PDFdata2(i,2),PDFdata2(i,3),PDFdata2(i,4)
				write(*, *) PDFdata2(i,1),PDFdata2(i,2),PDFdata2(i,3),PDFdata2(i,4)
            end do 
            write(*,*) 'read ,PDFdata(i,3) value finished!'
            close(401)	
